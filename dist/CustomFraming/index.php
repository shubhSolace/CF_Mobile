<?php
    ...
    if (isset($_REQUEST['demo'])&& $_REQUEST['demo'] == 1) {
        include_once('custom_framing_ng/index_demo_20230124.php');
		exit();
    }
    if (isset($_REQUEST['demo_angular'])&& $_REQUEST['demo_angular'] == 1) {
        include_once('custom_framing_ng/index_demo_only_angular.php');
		exit();
    }
    ...
    if ($name_meta_id) {
        ...
        // header("Location: https://www.arttoframe.com/collage-frames");
        // exit; 
    }
    /*
        if(strpos($mat_id, 'custom') !== false){
        } else{
            if($name_meta_id == '')
            {
            header("Location: https://www.arttoframe.com/collage-frames");
    ...
    {
        //echo "SELECT diploma_frames FROM `multi_mat` WHERE `id` = '".$_GET["mat_id"]."'";
    ...
    if ($_REQUEST['frame_code'] != '') {
        /*
        Updated this logic on 2022-03-08
        To redirect to default frame if frame_types.is_frame_inclusive frame code is not there in request.
        Task Reference.: https://arttoframes.teamwork.com/#/tasks/27366320?c=10740434
        */
    ...
    if(isset($_GET['mat_id']) && !empty($_GET['mat_id'])){
        ...
        // echo '<pre>';print_r($ka_getMatidrelatedInfo_ary);exit;
    ...
    // echo (is_numeric($ka_param['mat_id'])); exit;
    $ka_price_calclulations = ka_price_calclulations($ka_param);
    // echo '<pre>'; print_r($ka_price_calclulations); exit;
    /*================== START: Code added for matching page load price meta tag with price visible =======================*/
    ...
    if($diploma_flag == 1) {
        ...
        #$ka_price_calclulations['arrOrder']['paper_backing'] = 9; this updated on 20220208 to match with desktop config
    ...
    if (isset($_GET['mat_only']) && $_GET['mat_only'] == '1') {
        ...
        if($_GET['mat_color1']!=''){
            ...
            $zaius_Product_sku = "MAT-".$_GET['mat_color1']."-".$mat_size_schema;
        ...
        if($_GET['mat_color1']!='' && $_GET['mat_color2']!=''){
            ...
            $zaius_Product_sku = "MAT-".$_GET['mat_color1']."-".$_GET['mat_color2']."-".$mat_size_schema;
        ...
        if($_GET['mat_color1']!='' && $_GET['mat_color2']!=''  && $_GET['mat_color3']!=''){
            ...
            $zaius_Product_sku = "MAT-".$_GET['mat_color1']."-".$_GET['mat_color2']."-".$_GET['mat_color3']."-".$mat_size_schema;
    ...
    if (isset($_REQUEST['letterart'])) { 
        ...
        // echo 'inside_width='.$inside_w.'&inside_height='.$inside_h.'&matcode='.$_REQUEST['cut_id'].'&frame_id='.intval($_GET['frameId']).'&current_frame_code='.$default_frame_code.'&glass=yes&fitting=yes&thirdparty=0&mat1_id='.$mat1_id.'&mat2_id='.$mat2_id.'&mat3_id='.$mat3_id.'&printing=yes&glass_type='.$rowShowGlassSelected['id'].'&backing_type=2&debug=debug&cut_val=0&printing_type='.$printing_type.'&hardware_id=52&hardware_type=52&paper_backing=9&website=2&show_cost=1&cuts_width=4&cuts_height=6'.$image_string;
...
// start- added code for facebook pixel update sku field for WOM,collage and frames with mat -24jan2023
...
// start- added code for listrak tag for sku -27feb2023

if ($type=='no_mat') {
    
                    $zaius_Product_sku ="WOM-".$defalt_frame_info["code"].'-'.$selected_frame_size; 
                    $zaius_Product_type_id = 38;                          
                    $zaius_product_title   = "Picture Frames";  
          }elseif ($type=='collage_sm') {          
            
                    $zaius_Product_sku ="Collage-".$_GET["mat_id"];   
                    $zaius_Product_type_id = 78;                          
                    $zaius_product_title   = "Collage Frame"; 
    ...
    if(isset($mat_id) && is_numeric($mat_id))
         {
                    ...
                    $zaius_Product_sku ="FramesWithMat-".$_GET["frame_code"];   
                    $zaius_Product_type_id = 79;                          
                    $zaius_product_title   = "Frames With Mat";                  
          }
      } 
...
<!--  end- added code for facebook pixel update sku field for WOM,collage and frames with mat -24jan2023 -->
<?php
    if (isset($_REQUEST['letterart'])) { 
        $product_title = "Custom Framing - Letter Art";
        $zaius_Product_sku = "customframe-".$default_frame_code;
...
// Start- adding varaible and pushing data using dataLayer for listrak tag 27feb2023
var zaius_product_id = "<?php echo $zaius_Product_sku;?>";
var zaius_product_price =  "<?php echo $myTotalPrice;?>";
var zaius_product_url = window.location.href;
var zaius_cart_product_image_url =  "<?php echo "https://www.arttoframe.com".$picture."";?>";
var zaius_product_title =  "<?php echo $product_title;?>";
var zaius_product_quantity = '1';

dataLayer.push({'zaius_product_id':zaius_product_id});
dataLayer.push({'zaius_product_price':zaius_product_price});
dataLayer.push({'zaius_product_url':zaius_product_url});
dataLayer.push({'zaius_cart_product_image_url':zaius_cart_product_image_url});
dataLayer.push({'zaius_product_title':zaius_product_title});
dataLayer.push({'zaius_product_quantity':zaius_product_quantity});
 
// End- adding varaible and pushing data using dataLayer for listrak tag 27feb2023
// Start- adding varaible and pushing data using dataLayer for listrak tag for cart update 10mar2023

// $(window).bind('load click', function() {

// var zaius_product_id = "<?php// echo $zaius_Product_sku;?>";
// var zaius_product_price =  $('.price-txt').html();
// var zaius_product_price= zaius_product_price.replace(/[_$\W]+/g, "");
// var zaius_product_title =  $('.product-name').html();
// var zaius_product_url = window.location.href;
// var zaius_cart_product_image_url =  "<?php //echo "https://www.arttoframe.com".$picture."";?>";
// var zaius_product_quantity = $('.QuantityInputBx').val();
...
// commented below code of Listrak because we update code through api now below code is repeating the cart abonnement tag so we commented this code - 19April2023
...
//         { 
// echo'_ltk.SCA.AddItemWithLinks("'.$order_items[$i]['order_item_details_new'][0]['sku'].'", "'.$order_items[$i]['order_item_details_new'][0]['qty'].'","'.$order_items[$i]['order_item_details_new'][0]['line_item_total'].'","'.$order_items[$i]['page_title'].'","'.$order_items[$i]['order_item_details_new'][0]['preview_image'].'");';  
...
<!-- <script type="text/javascript">
  ...
  var Listrak_Cart_Total_Price='<?php //echo $cart_total; ?>'; 
  dataLayer.push({'Listrak_Cart_Total_Price':Listrak_Cart_Total_Price});
</script> -->


<!--  END- adding varaible and pushing data using dataLayer for listrak tag for cart update 10mar2023 -->
 ...
 <head>
     ...
     <script src="https://www.arttoframe.com/js/fiber/fiber.min.js"></script>
     <link href="<?php echo getCDNurl(1); ?>/css/jquery.selectareas.css?v=3.1" media="screen" rel="stylesheet" type="text/css" />
     <script src="https://www.arttoframe.com/js/jquery.selectareas.js?v=1.9" type="text/javascript"></script>
     ...
     <script type="text/javascript">
         $(document).ready(function(){
             $.ajax({
                ...
                // url : 'https://www.arttoframe.com/demo_custom_framing_ng/assets/page_visit_count.php',
         ...
         //     var textInput = $("#customWidth");

         //     FastClick.attach(document.getElementById('sizemodal'));
         //     Array.prototype.forEach.call($('#showsizepopu'), function (testEl) {
        ...
        ?>
        // console.log("Loading start <?php echo "Date: $date:" . $date_array[0]; ?>");

        // $plexiGlassImg = getCDNurl(3).'/products/glaze/new/Plexi-Regular.png';
        //  print_r($plexiGlassImg);
         localStorage.setItem('defaultParam',JSON.stringify(<?php echo json_encode($_REQUEST) ?>));
        //  localStorage.setItem('frames',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getFrameDataAngular.json')?> ));
        //  localStorage.setItem('mats',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getMatDetailsAngular.json')?> ));
        //  localStorage.setItem('glass',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getGlassData.json')?> ));
        //  localStorage.setItem('Hardware',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getHardware.json')?> ));
        //  localStorage.setItem('FrameFilterColorsName',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getFrameFilterColorsName.json')?> ));
        //  localStorage.setItem('MatFilterColors',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getMatFilterColors.json')?> ));
        //  localStorage.setItem('collage_opening',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/collage_opening.json')?> ));
        //  localStorage.setItem('facncyShape',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getDecorativeMatDetailsAngular.json')?> ));
        //  localStorage.setItem('vgroove',JSON.stringify(<?php echo file_get_contents($_SERVER['DOCUMENT_ROOT'].'/ng/assets/Json/getVgooveDetailsAngular.json')?> ));
        
        //  $plexiGlassImg =  <?php  echo getCDNurl(2); ?>"/shadowbox_product/runtime.e227d1a0e31cbccbf8ec.js?v1.4"
        // localStorage.setItem('plexiglassimage',"<?php  echo getCDNurl(2); ?>/products/glaze/new/Plexi-Regular.png" );
         ...
         // $date_array = explode(" ",$micro_date);
         // $date = date("Y-m-d H:i:s",$date_array[1]); ?>
         // console.log("Loaded <?php echo "Date: $date:" . $date_array[0]; ?>");
         // for Google anylytics
 ...
 <body>
     ...
     <script type="text/javascript" src="<?php echo getCDNurl(1); ?>/custom_framing_ng/runtime.4cd17dad50b0f888773e.js" integrity="sha384-WCO1dM1VIjdz4wJR0FG7yGtGylSMdwCQDP5MoFjrc/u8970XcFh6zwXdjG76eCDS" crossorigin="anonymous"></script>
     <script type="text/javascript" src="<?php echo getCDNurl(1); ?>/custom_framing_ng/es2015-polyfills.1905b039bfbf91927171.js" integrity="sha384-vvAx8uXZkHN399MNUPXy8/mt/cHtIdgGXT0ONVaN/7g4ngWqvLV2HKG1k1A+etKS" crossorigin="anonymous" nomodule></script>
     <script type="text/javascript" src="<?php echo getCDNurl(1); ?>/custom_framing_ng/polyfills.b7c5b10606aa202be8fb.js" integrity="sha384-UAoMh3xWCCXKWZaNJn9UBNBjyimlBWhtFnqJAqlDgw86H6NtmkcUX6wX/yFxmhMb" crossorigin="anonymous"></script>
     <script type="text/javascript" src="<?php echo getCDNurl(1); ?>/custom_framing_ng/main.d6457195837225e0ac28.js" integrity="sha384-WvYI1QrMfZmYG6bDg0O2kA3A0/QOx4jUcl0XHPLCIsrgsclnpLYmJw0QVJ97LZCe" crossorigin="anonymous"></script>
     <?php
